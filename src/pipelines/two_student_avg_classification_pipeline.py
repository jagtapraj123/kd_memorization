...
class GenPipeline:
    def __init__(
        ...
        log_files_path: str,
        teacher_models: List[torch.nn.Module] = None,
        cuda_num: int = 0,
        **kwargs
    ):
        ...
        if self.teacher_models is not None:
            for i in range(len(self.teacher_models)):
                self.teacher_models[i].to(self.device)
...
class PipelineS1(GenPipeline):
    ...
    def train(
        ...
        print("Teachers", self.teacher_models)
        self.epochs = num_epochs
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for x_train, y_train, idx in pbar:
                ...
                if teacher_weightage > 0:
                    if self.teacher_models is not None:
                        teacher_out = self.teacher_models[0](x)
                        y_teacher = (teacher_out - torch.mean(teacher_out, dim=1, keepdim=True)) / torch.std(teacher_out, dim=1, keepdim=True)
                        for i in range(1, len(self.teacher_models)):
                            teacher_out = self.teacher_models[i](x)
                            y_teacher += (teacher_out - torch.mean(teacher_out, dim=1, keepdim=True)) / torch.std(teacher_out, dim=1, keepdim=True)
                        y_teacher /= len(self.teacher_models)
                    else:
                        raise RuntimeError("Using un-specified teacher model")
...
class PipelineS2(GenPipeline):
    def __init__(
        self,
        name: str,
        model_s1s: List[torch.nn.Module],
        ...
        **kwargs
        ...
        self.model_s1s = model_s1s
        for i in range(len(self.model_s1s)):
            self.model_s1s[i].to(self.device)
    ...
    def train(
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for x_train, y_train, idx in pbar:
                ...
                # Forward pass
                s1_out = self.model_s1s[0](x)
                # y_pred_s1 = torch.div(y_pred_s1.T, torch.max(y_pred_s1, dim=1).values).T
                y_pred_s1 = (s1_out - torch.mean(s1_out, dim=1, keepdim=True)) / torch.std(s1_out, dim=1, keepdim=True)
                for i in range(1, len(self.model_s1s)):
                    s1_out = self.model_s1s[i](x)
                    y_pred_s1 += (s1_out - torch.mean(s1_out, dim=1, keepdim=True)) / torch.std(s1_out, dim=1, keepdim=True)
                y_pred_s1 /= len(self.model_s1s)
                ...
                target = y_teacher - y_pred_s1

                # Clearing previous epoch gradients
                optimizer.zero_grad()

                # Calculating loss
                loss = loss_func_with_grad(y_pred, target)
                """
                if teacher_weightage == 0:
                    loss = loss_func_with_grad(y_pred, y_truth)
                elif teacher_weightage == 1:
                ...
                for i in range(len(idx)):
                    grad = torch.autograd.grad(
                        loss_func_with_grad(y_pred[i], target[i]),
                        self.model.fc.parameters(),
                        create_graph=True,
                    )
                    # print(sum(p.numel() for p in grad))
                ...
                y_preds += list(
                    torch.argmax(y_pred_s1 + y_pred, dim=1).cpu().detach().numpy()
                )
                ...
                for test_name, test_loader in self.test_loaders.items():
                    ...
                    with torch.no_grad():
                        for x_test, y_test in tqdm(
                            ...
                            y_preds += list(
                                torch.argmax(y_pred_s1 + y_pred, dim=1)
                                .cpu()
                                .detach()
                                .numpy()
                            )
